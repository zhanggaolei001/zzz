...
!(async () => {
  ...
  var s = randomNum(10, 100);
  console.log(`随机等待:${s}秒`)
  await $.wait(s * 1000)
...
async function signInit() {
  return new Promise(resolve => {
    const turl = taskUrl('speedSignInit', {
      "activityId": "8a8fabf3cccb417f8e691b6774938bc2",
      "kernelPlatform": "RN",
      "inviterId": "WJ/Vk7yB4bI55sclaJwvRA=="
    });
    console.log(turl);
    $.get(turl, async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          if (resp.statusCode == 403) {
            console.log(`ip被禁,等待${sleep_seconds/1000/60}分钟后继续执行`);
...
async function taskList() {
  return new Promise(resolve => {
    ...
    var turl = taskUrl('ClientHandleService.execute', {
      "version": "3.1.0",
...
async function queryJoy() {
  return new Promise(resolve => {
    const turl = taskUrl('ClientHandleService.execute', { "method": "queryJoyPage", "data": { "channel": 1 } });
    console.log(turl);
    $.get(turl,
      async (err, resp, data) => {
        try {
...
async function rewardTask(id, taskId) {
  return new Promise(resolve => {
    const turl = taskUrl('ClientHandleService.execute', {
...
async function cash() {
  return new Promise(resolve => {
    var turl = taskUrl('MyAssetsService.execute',
...
// 大转盘做任务
async function apDoTask(taskType, taskId, channel, itemId) {
...
// 红包大富翁
async function richManIndex() {
  return new Promise(resolve => {
    var turl = taskUrl('richManIndex', { "actId": "hbdfw", "needGoldToast": "true" });
    console.log(turl);
    $.get(turl, async (err, resp, data) => {
      try {
        if (err) {
...
// 红包大富翁
async function shootRichManDice() {
  return new Promise(resolve => {
    var turl = taskUrl('shootRichManDice', { "actId": "hbdfw" });
    console.log(turl);
    $.get(turl, async (err, resp, data) => {
      try {
        if (err) {
