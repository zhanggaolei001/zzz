  ...
  for (let [index, value] of cookiesArr.entries()) {
    try {
      ...
      for (let i = 0; i < 3; i++) {
        try {
          log = await getLog()
          res = await api('h5launch', { followShop: 0, random: 19063111, log: log, sceneid: 'JLHBhPageh5' })
    ...
    try {
      ...
      for (let t of res.data.result.redpacketConfigFillRewardInfo) {
        if (t.packetStatus === 1) {
          if (autoOpen) {
            log = await getLog()
            res = await api('h5receiveRedpacketAll', {random: 19063111, log: log, sceneid: 'JLHBhPageh5' })
      ...
      for (let code of shareCodes) {
        ...
        if (!fullCode.includes(code) && code !== me) {
          ...
          for (let i = 0; i < 5; i++) {
            if (success) break
            log = await getLog()
            res = await api('jinli_h5assist', { "redPacketId": code, "followShop": 0,random: 19063111, log: log , sceneid: 'JLHBhPageh5' })
            if (res.rtn_code !== 0) {
