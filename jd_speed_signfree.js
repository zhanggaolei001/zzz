...
//签到
async function sign(cookie, orderId) {
    return new Promise(resolve => {
        const options = {
...
//提现
async function cash(orderId) {
    return new Promise(resolve => {
        // console.debug('cash orderId:', orderId)
        $.post(taskPostUrl("signFreePrize", { "linkId": "PiuLvM8vamONsWzC0wqBGQ", "orderId": orderId, "prizeType": 2 }), async (err, resp, data) => {
            try {
                if (err) {
                    console.error(`${JSON.stringify(err)}`)
                    //提现失败通知
                    notify.sendNotify(`${$.name}+提现失败`, JSON.stringify(err));
                    ...
                    let msg_temp
                    if (data.success) {
                        msg_temp = $.UserName + $.productName + ' 提现成功'
                        notify.sendNotify(`${$.name}+提现成功`, msg_temp);
                        //提现成功通知
                    } else {
                        //提现失败通知
                        msg_temp = $.UserName + $.productName + (data.errMsg || '未知错误')
                        notify.sendNotify(`${$.name}+提现失败`, msg_temp);
                    }
...
function taskPostUrl(function_id, body) {
    return {
        ...
        headers: {
            ...
            "referer": "https://signfree.jd.com/?activityId=" + "PiuLvM8vamONsWzC0wqBGQ"
        }
    }
}
