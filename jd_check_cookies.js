...
var cookieFile = "/scripts/panel/config/config.sh"
const os = require('os');
...
switch (platform) {
  case 'darwin':
    ...
    break;
  case 'linux':
    cookieFile = "/scripts/panel/config/config.sh"
    console.log('Linux')
    break;
  case 'win32':
    cookieFile = "./panel/config/config.sh"
    console.log('Windows')
    break;
  default:
...
lines.forEach(line => {
  try {
    if (line.includes("pt_key") && !cookiesArr.includes(line)) {
...
!(async () => {
  ...
  if ($.isNode()) {
    fs.writeFileSync(cookieFile, cookiesArr.join("\n") + "\n");
  }
