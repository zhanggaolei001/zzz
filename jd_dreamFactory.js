...
!(async () => {
  ...
  let hour = new Date().getHours(); //拿到当前小时
  if (hour > 10) {
    for (let i = 0; i < cookiesArr.length; i++) {
      if (cookiesArr[i]) {
        cookie = cookiesArr[i];
        ...
        if (cookiesArr && cookiesArr.length >= ($.tuanNum || 0) && $.canHelp) {
          console.log(`\n账号${$.UserName} 内部相互进团\n`);
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (
                  ...
                  $.log(
                    `\n本次发电机电力集满分享后${data.data.nextCollectDoubleFlag === 1 ? "可" : "不可"
                    }获得双倍电力，${data.data.nextCollectDoubleFlag === 1
                      ? "故目前不收取电力"
                      : "故现在收取电力"
                    }\n`
                  );
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                console.log(
                  `\n\n你今日还能帮好友打工（${assistNumMax - assistListToday.length || 0
                  }/${assistNumMax}）次\n\n`
                );
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  if ((production.investedElectric / production.needElectric) > 0.95) {
                    $.log(`\n\n进度超过95%,不再自动生产`);
                    ...
                    if (production["exchangeStatus"] === 3) {
                      ...
                      if (new Date().getHours() === 9) {
                        ...
                        allMessage += `【京东账号${$.index}】${$.nickName
                          ...
                          }兑换已超时，请选择新商品进行制造${$.index !== cookiesArr.length ? "\n\n" : ""
                          }`;
                      }
                    }
                    // await exchangeProNotify()
...
async function stealFriend() {
  ...
  for (let i = 0; i < $.friendList.length; i++) {
    ...
    console.log(
      `\n开始收取第 ${i + 1} 个好友 【${$.friendList[i]["nickName"]
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (
                  data["ret"] === 10218 &&
                  ($.index === 1 || cookiesArr.length === $.index)
                ) {
                    ...
                    if (item2.encryptPin === $.encryptPin) {
                      if (item2.receiveElectric && item2.receiveElectric > 0) {
                        console.log(
                          `${new Date(
                            item2.joinTime * 1000
                          ).toLocaleString()}参加团主【${item2.nickName
                          }】的奖励已经领取成功`
                        );
                      } else {
                        console.log(
...
async function exchangeProNotify() {
  ...
  for (let shel of $.shelvesList) {
    console.log(
      `${shel["name"]}    ${shel["exchangeStatus"] === 1
        ? "未兑换"
...
function readShareCode() {
  ...
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `http://share.turinglabs.net/api/v3/jxfactory/query/${randomCount}/`,
        timeout: 10000,
      },
      (err, resp, data) => {
        try {
...
function shareCodesFormat() {
  return new Promise(async (resolve) => {
    ...
    if ($.shareCodesArr[$.index - 1]) {
      ...
      $.newShareCodes = ["kOzG-mZxofSXXukyON5Veg==", "eD-KASmvNcMBsIts31TWew==", "Z_VRPrP702CAAIQks4rnHw==", "Ctmu6bo7nZrgWURNTPtndg=="];
    }
...
function requireConfig() {
  return new Promise(async (resolve) => {
    ...
    $.shareCodesArr = ["rMaB09l0vcDvNyCcG60sww=="];
    if ($.isNode()) {
...
function taskTuanUrl(functionId, body = "", stk) {
  let url = `https://m.jingxi.com/dreamfactory/tuan/${functionId}?${body}&_time=${Date.now()}&_=${Date.now() + 2
...
function taskurl(functionId, body = "", stk) {
  let url = `${JD_API_HOST}/dreamfactory/${functionId}?zone=dream_factory&${body}&sceneval=2&g_login_type=1&_time=${Date.now()}&_=${Date.now() + 2
    }&_ste=1`;
  url += `&h5st=${decrypt(Date.now(), stk, "", url)}`;
  if (stk) {
    url += `&_stk=${encodeURIComponent(stk)}`;
...
function newtasksysUrl(functionId, taskId, stk) {
  let url = `${JD_API_HOST}/newtasksys/newtasksys_front/${functionId}?source=dreamfactory&bizCode=dream_factory&sceneval=2&g_login_type=1&_time=${Date.now()}&_=${Date.now() + 2
    }&_ste=1`;
  if (taskId) {
    url += `&taskId=${taskId}`;
  }
