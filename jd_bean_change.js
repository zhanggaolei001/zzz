...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    try {
      if (cookiesArr[i]) {
        ...
        item.index = $.index;
        item.beanCount = $.beanCount;
        item.incomeBean = $.incomeBean;
        item.expenseBean = $.expenseBean;
        item.todayIncomeBean = $.todayIncomeBean;
        item.errorMsg = $.errorMsg;
        item.isLogin = $.isLogin;
        item.nickName = $.nickName;
        item.message = $.message;
        item.balance = $.balance;
        item.expiredBalance = $.expiredBalance;
        item.JdzzNum = $.JdzzNum;
        item.JdMsScore = $.JdMsScore;
        item.JdFarmProdName = $.JdFarmProdName;
        item.JdtreeEnergy = $.JdtreeEnergy;
        item.jxFactoryInfo = $.jxFactoryInfo;
        item.ddFactoryInfo = $.ddFactoryInfo;
        item.JdtreeTotalEnergy = $.JdtreeTotalEnergy;
        item.JdwaterTotalT = $.JdwaterTotalT;
        item.JdwaterD = $.JdwaterD;
        item.JDwaterEveryDayT = $.JDwaterEveryDayT;
        item.JDtotalcash = $.JDtotalcash;
        item.JDEggcnt = $.JDEggcnt;
        item.Jxmctoken = $.Jxmctoken;
        item.petInfo = $.petInfo;
        item.jxRed = ($.jxRed != undefined && $.jxRed > 0) ? $.jxRed : "无";
        item.jsRed = ($.jsRed != undefined && $.jsRed > 0) ? $.jsRed : "无"; 
        item.jdRed = ($.jdRed != undefined && $.jdRed > 0) ? $.jdRed : "无";
        item.jdhRed = ($.jdhRed != undefined && $.jdhRed > 0) ? $.jdhRed.toFixed(2) : "无";
        item.jxRedExpire = ($.jxRedExpire != undefined && $.jxRedExpire > 0) ? $.jxRedExpire.toFixed(2) : "无";
        item.jsRedExpire = ($.jsRedExpire != undefined && $.jsRedExpire > 0) ? $.jsRedExpire.toFixed(2) : "无";
        item.jdRedExpire = ($.jdRedExpire != undefined && $.jdRedExpire > 0) ? $.jdRedExpire.toFixed(2) : "无";
        item.jdhRedExpire = ($.jdhRedExpire != undefined && $.jdhRedExpire > 0) ? $.jdhRedExpire.toFixed(2) : "无";
        items.push(item) 
      }
    } catch (error) {
      console.log(error);
  ...
  if ($.isNode() && allMessage) {
    ...
    var obj = items;//解析json字符串为json对象形式
    var trStr = '';//动态拼接table

    for (var i = 0; i < obj.length; i++) {//循环遍历出json对象中的每一个数据并显示在对应的td中
      trStr += '<tr class="example">';//拼接处规范的表格形式
      trStr += '<td>' + obj[i].index + '</td>';//数据表的主键值
      trStr += '<td>' + obj[i].UserName + '</td>';//对应数组表的字段值
      trStr += '<td>' + obj[i].todayIncomeBean + '京豆 🐶</td>';
      trStr += '<td>' + obj[i].incomeBean + '京豆 🐶</td>';
      trStr += '<td>' + obj[i].expenseBean + '京豆 🐶</td>'; 
      trStr += '<td>' + obj[i].beanCount + '京豆 🐶</td>';
      trStr += '<td>' + `${obj[i].balance}(总过期${obj[i].expiredBalance})元 🧧` + '</td>';
      trStr += '<td>' + `${obj[i].jxRed}(将过期${obj[i].jxRedExpire})元` + '</td>';
      trStr += '<td>' + `${obj[i].jsRed}(将过期${obj[i].jsRedExpire})元` + '</td>';
      trStr += '<td>' + `${obj[i].jdRed}(将过期${obj[i].jdRedExpire})元` + '</td>';
      trStr += '<td>' + obj[i].JDEggcnt + '枚鸡蛋</td>';
      trStr += '<td>' + obj[i].JDtotalcash/10000 + '元的金币</td>';
      trStr += '<td>' + obj[i].JdzzNum/10000 + '元的金币</td>';
      trStr += '<td >' + obj[i].JdMsScore/1000 + '元的秒秒币</td>';
      var str = "";
      if (obj[i].JdtreeEnergy != 0) {
        str = `${obj[i].JdFarmProdName},进度${((obj[i].JdtreeEnergy / obj[i].JdtreeTotalEnergy) * 100).toFixed(2)}%`;
        if (obj[i].JdwaterD != 'Infinity' && obj[i].JdwaterD != '-Infinity') {
          str += `,${obj[i].JdwaterD === 1 ? '明天' : obj[i].JdwaterD === 2 ? '后天' : obj[i].JdwaterD + '天后'}可兑🍉`;
        } else {
          str += ``;
        }
      } else {
        str += `${obj[i].JdFarmProdName}`;
      }
      trStr += '<td>' + str + '</td>';
      trStr += '<td>' + obj[i].jxFactoryInfo + '</td>';
      trStr += '<td>' + obj[i].ddFactoryInfo + '</td>';
      trStr += '<td>' + obj[i].petInfo + '</td>'; 

      trStr += '<td>' + `${obj[i].jdhRed}(将过期${obj[i].jdhRedExpire})元` + '</td>';
      //当前总红包： 
      trStr += '</tr>';
    }
    var table = `<table class="vertical">${tableHeader}<tbody>${trStr}</tbody></table>`.toString()
 
    var html = ` 
     <!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
      <html xmlns="http://www.w3.org/1999/xhtml">
      <head> 
      <title>京东资产变动通知${(new Date()).toUTCString()}</title>
      <style type="text/css">
          table {
...
<body>
 ${table} 
     ...
     `
    console.log(html);
    await notify.sendNotify(`${$.name}`, `${html}`, { url: `https://bean.m.jd.com/beanDetail/index.action?resourceValue=bean` })
  }
})()
  .catch((e) => {
    $.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
...
async function showMsg() {
  ...
  if (initPetTownRes.code === '0' && initPetTownRes.resultCode === '0' && initPetTownRes.message === 'success') {
    ...
    if (response.resultCode === '0') {
      ...
      $.petInfo = `${$.petInfo.goodsInfo.goodsName},勋章${response.result.medalNum}/${response.result.medalNum + response.result.needCollectMedalNum}块(${response.result.medalPercent}%)`
    }
  }
  ReturnMessage += `🧧🧧🧧🧧红包明细🧧🧧🧧🧧`;
...
function cash() {
  return new Promise(resolve => {
    const newLocal = taskcashUrl('MyAssetsService.execute',
