...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      $.nickName = '';
      message = '';
      console.log(`===${$.index}:${$.UserName},开始提供助力:=====`);
      await helpFriends(); 
    }
  }
...
async function helpFriends() {
  for (let code of $.newShareCodes) {
    if ( pass.includes(code)) {
      continue;
    }
    if (!code) continue
    const helpRes = await jdfactory_collectScore(code); 

    if (helpRes.code === 0 &&  helpRes.data.bizCode === -7||helpRes.data.bizCode===-1001) {
      console.log(helpRes.data.bizMsg+",跳出");
      break
    }
    if (helpRes.code === 0 &&  (helpRes.data.bizCode === -9||helpRes.data.bizCode ===-7001)) {
      //todo:从$.newShareCodes中删除code,已经满了,后面别助力了.
      console.log(helpRes.data.bizMsg+",跳出");
      pass.push(code)
      continue;
    }
    if (helpRes.code === 0 &&  (helpRes.data.bizCode ===-7001)) {
      //todo:从$.newShareCodes中删除code,已经满了,后面别助力了.
      console.log(helpRes.data.bizMsg+",跳出"); 
      break
    }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.data.bizCode === 0) {
              ...
              $.taskVos.map(item => {
                if (item.taskType === 14) {
                  $.newShareCodes.push(item.assistTaskDetailVo.taskToken)
